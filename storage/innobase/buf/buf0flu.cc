...
void set_average() {
  ...
  /* Both LRU managers and page cleaners record into the same per-instance
  accumulator. Drain it regardless of the current owner so a mode transition
  cannot lose or defer the old owner's final interval. */
  buf_pool_t::lru_flush_stat_t lru_stat{};
  for (ulint i = 0; i < srv_buf_pool_instances; i++) {
    buf_pool_t *const buf_pool = buf_pool_from_array(i);
    mutex_enter(&buf_pool->flush_state_mutex);
    const auto &instance_stat = buf_pool->lru_flush_stat;
    ...
    lru_stat.n_lru_passes += instance_stat.n_lru_passes;
    lru_stat.lru_flush_time_ms += instance_stat.lru_flush_time_ms;
    buf_pool->lru_flush_stat = {};
    mutex_exit(&buf_pool->flush_state_mutex);
  }

  /* Publish exact cumulative totals/counts. The per-call monitor's current
  value is the interval average; its max is the largest batch in the interval.
  The coordinator remains the only monitor writer. */
  ...
  lru_tm += lru_stat.lru_flush_time_ms;
  lru_pass += static_cast<ulint>(lru_stat.n_lru_passes);

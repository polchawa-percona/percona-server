...
static MYSQL_SYSVAR_BOOL(
    ...
    PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_READONLY,
    "Enable per-buffer-pool LRU manager threads that flush the LRU tail "
    "to refill the free list. Always active during recovery; after recovery "
    "this setting controls whether the threads run. When disabled "
    "(the default), page cleaners perform LRU flushing instead.",
    nullptr, nullptr, false);
